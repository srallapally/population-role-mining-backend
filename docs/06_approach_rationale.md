...
The pipeline already implements this hybrid. Leiden finds communities in Step 5. Step 6 then runs entitlement prevalence analysis within each community's own membership — not against the session-wide population. For every entitlement in the residual matrix, the fraction of that community's members who hold it determines its tier: role-defining (≥ coverageThreshold), common-not-universal (≥ softThreshold and < coverageThreshold), or excluded (< softThreshold). This is the two-tier entitlement system. Community detection finds who belongs together; per-community prevalence analysis determines which entitlements actually define the role versus which are incidental.

The one thing not yet implemented that would go further is full intra-community association rule mining — finding entitlement co-occurrence patterns within each community rather than just prevalence thresholds. That would detect, for example, that entitlements A and B always appear together within the community even if neither individually meets coverageThreshold. That is a genuine future enhancement. Per-community prevalence analysis is already in place and operational.
