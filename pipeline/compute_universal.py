# backend/pipeline/compute_universal.py
import uuid

import numpy as np
from scipy.sparse import csr_matrix

from data.loader import get_entitlements
...
def compute_universal(
    ...
    for i, p in enumerate(prevalence):
        if coverage_threshold <= p < universal_threshold:
            ent_id = idx_to_ent[i]
            meta = _get_ent_meta(ent_id)
            high_residual_prevalence.append({
                "entId": ent_id,
                "displayName": meta.get("ent_name", ent_id),
                "appName": meta.get("app_id", ""),
    ...
    entitlements_df = get_entitlements()
    ent_lookup = entitlements_df.set_index("ent_id").to_dict("index") if not entitlements_df.empty else {}
...
def _build_layer1_role(
    ...
    for c, ent_id in zip(cluster_cols, ent_ids):
        ...
        ent_meta.append({
            "entId": ent_id,
            "displayName": meta.get("ent_name", ent_id),
            "appId": meta.get("app_id", ""),
            "appName": meta.get("app_id", ""),
            "entitlementType": meta.get("EntitlementTypeName", ""),
            "criticality": "",
...
def _get_ent_meta(ent_id: str) -> dict:
    df = get_entitlements()
    row = df[df["ent_id"] == ent_id]
    if row.empty:
        return {}
