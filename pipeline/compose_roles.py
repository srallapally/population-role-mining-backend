# backend/pipeline/compose_roles.py
import uuid
...
def compose_roles(
    ...
    ent_lookup = (
        entitlements_df.set_index("ent_id").to_dict("index")
        if not entitlements_df.empty else {}
    )
...
def _build_ent_meta(
    ...
    for cols, tier in [(role_defining_cols, "role_defining"),
        ...
        for c in cols:
            ...
            tier_meta.append({
                "entId": ent_id,
                "displayName": info.get("ent_name", ent_id),
                "appId": info.get("app_id", ""),
                "appName": info.get("app_id", ""),
                "entitlementType": info.get("EntitlementTypeName", ""),
                "criticality": "",
